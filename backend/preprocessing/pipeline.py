...
def check_image_quality(image_bgr: np.ndarray, config: QualityConfig | None = None) -> QualityResult:
    ...
    if reasons:
        return QualityResult(False, "Image quality is insufficient for reliable screening. Please upload a clearer retinal image.", blur_score, brightness, retinal_area, tuple(reasons))
    return QualityResult(True, f"Acceptable (clarity {blur_score:.1f}, retinal area {retinal_area:.2f})", blur_score, brightness, retinal_area, ())
...
def preprocess_image(image_bgr: np.ndarray, input_size: int = 224) -> tuple[np.ndarray, Any, dict[str, Any]]:
    ...
    ])(Image.fromarray(enhanced_rgb)).unsqueeze(0)
    metadata = {
        "input_size": input_size,
        "operations": ["retinal_region_crop", "LAB_CLAHE", "resize", "ImageNet_normalization"],
        "visualization_note": "Preprocessing improves visibility for model input; it is not a clinical enhancement claim.",
    }
    return enhanced_rgb, tensor, metadata


def basic_preprocess_image(image_bgr: np.ndarray, input_size: int = 224) -> tuple[np.ndarray, Any, dict[str, Any]]:
    ...
    return rgb, tensor, {"input_size": input_size, "operations": ["resize", "ImageNet_normalization"], "variant": "baseline"}
